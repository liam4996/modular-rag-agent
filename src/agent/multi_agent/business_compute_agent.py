...
Outputs metrics, charts, tables, and rendered reports. Pure Python — no LLM.
"""
...
from .tools.data_processor import (
    build_comparison_table, detect_outliers, to_markdown_table,
)
...
class BusinessComputeAgent:
    ...
    def compute(
        self, state: Any, operation: str = "calculate_metrics",
    ...
    def _calc_metrics(self, state: Any, params: Dict) -> None:
        ...
        state.add_to_blackboard("computed_results", {
            **state.blackboard.get("computed_results", {}),
            "metrics": all_metrics, "type": "calculate_metrics",
        }, "business_compute")
        state.add_execution_trace({
            "agent": "business_compute", "action": "calculate_metrics",
    ...
    def _compare(self, state: Any, params: Dict, session_id: str) -> None:
        ...
        state.add_to_blackboard("computed_results", {
            **state.blackboard.get("computed_results", {}),
            "comparison_table": table_md, "comparison_data": compare_data,
            "outliers": outliers, "type": "compare_companies",
        }, "business_compute")
        state.add_to_blackboard("chart_paths", state.chart_paths + charts, "business_compute")
        state.add_execution_trace({
            "agent": "business_compute", "action": "compare_companies",
            "companies": len(compare_data), "charts_generated": len(charts),
        })
    ...
    def _visualize(self, state: Any, params: Dict, session_id: str) -> None:
        ...
        state.add_execution_trace({
            "agent": "business_compute", "action": "visualize",
            "charts_generated": len(charts),
        })
    ...
    def _render_report(self, state: Any, params: Dict, session_id: str) -> None:
        ...
        chart_refs = [c for c in charts if isinstance(c, dict)]
        all_valuation = {}
        all_profitability = {}
        all_growth = {}
        all_solvency = {}
        ...
        if report_type == "industry_comparison":
            report = self.renderer.render_industry_comparison(
                symbols=symbols, comparison_table=comparison_table, charts=chart_refs)
        else:
            report = self.renderer.render_earnings_review(
                ...
                valuation=all_valuation, profitability=all_profitability,
                growth=all_growth, solvency=all_solvency,
                comparison_table=comparison_table, charts=chart_refs)
        state.add_to_blackboard("generated_report", report, "business_compute")
        state.add_execution_trace({
            "agent": "business_compute", "action": "generate_report",
            "report_type": report_type, "report_length": len(report),
        })
