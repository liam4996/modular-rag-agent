...
class OpenOriginalDocumentTool:
    ...
    def parse_chunk_id(chunk_id: str) -> Dict[str, Any]:
        ...
        # The format is: {source_hash}_{index:04d}_{content_hash}
        # source_hash is 8 hex chars, index is 4 digits, content_hash is 8 hex chars
        # But doc_id could also be longer (from DocumentChunker, doc_id_full_index_contenthash)
        # Try to match the last two underscore-separated parts as index and content_hash
        if len(parts) >= 2:
            # The last part should be content_hash (8 hex chars)
            content_hash = parts[-1] if len(parts[-1]) == 8 else ""
            # The second-to-last part should be the 4-digit index
            if len(parts) >= 2 and len(parts[-2]) == 4 and parts[-2].isdigit():
                chunk_index = int(parts[-2])
                source_hash = "_".join(parts[:-2])
            else:
                # Fallback: the first element might be a full doc_id and the
                # index/content_hash split might be different
                # Try: doc_id format is {hash[:8]}_{index}_{content_hash[:8]}
    ...
    def _find_chunks_by_doc_hash(
        ...
        # Chunk IDs format: {source_hash}_{index:04d}_{content_hash}
        # All chunks from the same document share the same source_hash prefix.
        # This is the most reliable fallback strategy.
        try:
    ...
    def get_document_content(
        ...
        # Resolve doc_hash from chunk_id if needed
        ...
        if total_chars > max_chars:
            # Truncate but try to include highlighted chunks
            # Find highlighted chunk positions
            h_positions = []
            ...
            if h_positions:
                # Window around the first highlighted chunk
