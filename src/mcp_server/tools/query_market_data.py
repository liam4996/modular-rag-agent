...
Covers A-shares (Tushare primary + akshare fallback), US stocks and HK stocks (yfinance).
...
class QueryMarketDataTool:
    ...
    Data sources:
    - A-shares: Tushare (primary) + akshare (fallback)
    ...
    def __init__(
        ...
        # Tushare 作为 A 股主数据源
        try:
            from src.agent.multi_agent.tools.tushare_client import TushareClient
    ...
    def _fetch_a_share_quote(self, symbols: List[str]) -> List[MarketQuote]:
        """A 股行情 — 使用 TushareClient fetch_with_fallback（含重试+akshare降级）"""
        if not self._tushare:
            logger.warning("TushareClient not available, no A-share data")
            return []

        result = self._tushare.fetch_with_fallback("quote", symbols)
        ...
        if not data:
            if source == "none":
                logger.warning("A-share quote unavailable: both Tushare and akshare failed")
            return []

        logger.info(f"A-share quote source: {source} (trace: {trace_info})")
        results = []
        for item in data:
    ...
    def _fetch_a_share_fundamentals(self, symbols: List[str]) -> List[MarketFundamentals]:
        """A 股基本面 — 使用 TushareClient fetch_with_fallback（含重试+akshare降级）"""
        if not self._tushare:
            logger.warning("TushareClient not available, no A-share fundamentals")
            return []

        result = self._tushare.fetch_with_fallback("fundamentals", symbols)
        data = result.get("data", [])
        source = result.get("source", "none")

        if not data:
            if source == "none":
                logger.warning("A-share fundamentals unavailable: both Tushare and akshare failed")
            return []

        logger.info(f"A-share fundamentals source: {source}")
        results = []
        for item in data:
    ...
    def _fetch_a_share_history(self, symbols: List[str], period: str) -> List[MarketHistory]:
        """A 股 K 线 — Tushare (重试) → akshare (备选)"""
        if self._tushare:
            result = self._tushare.fetch_with_fallback("history", symbols, period=period)
            data = result.get("data", [])
            if data:
                results = []
                for item in data:
